...
def apply-private-source [
    ...
    if $policy != "push-local" {
        let protected = (protected-conflicts $data_root)
        if not ($protected | is-empty) {
            print-protected-conflicts $protected
            error make {
                msg: "Protected configuration differs from the private source. Resolve it with `dotresolve` before a private-authoritative apply."
            }
        }
    }
...
def main [
    ...
    if $validate {
        ...
        ^nu --no-config-file ($TOOLS_ROOT | path join "scripts" "validate-project.nu")
